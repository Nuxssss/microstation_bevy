...
pub struct PrototypeManager {
    ...
    pub tile_prototypes: HashMap<String, TilePrototype>
    //TODO other protos
}

impl PrototypeManager {
    pub fn spawn_entity(&self, id: &str, position: Option<Position>, commands: &mut Commands) -> Option<Entity> { //TODO переделать в Result
        ...
        for comp in &proto.components {
            match comp {
                YamlComponent::MetaData(c) => { entity.insert(c.clone()); }
                YamlComponent::Transform(c) => { entity.insert(c.clone()); }
                YamlComponent::Sprite(c) => {entity.insert(c.clone()); },
                YamlComponent::IconSmooth(c) => {entity.insert(c.clone()); },
                YamlComponent::Item(c) => { entity.insert(c.clone()); }
                YamlComponent::Other => {
        ...
        info!("[Spawn] Entity {:?} components: {:?}", entity.id(), entity.id());
        Some(entity.id())
    }
}
